    ...
    ! Apr 2014: FYCR: Updated for interacting neutrinos
...
1   if (khi-klo.gt.1) then
    ...
    type EvolutionVars
        ...
        real(dl) has_nu_relativistic_pir_pert
        real(dl) pir_massive(max_nu,nqmax0)
    ...
    integer ind, nu_i, ind3,inq3,iq_iter,mass_int,l_iter
    !FYCR
    ...
    real(dl) noSwitch, smallTime, tau_p, tau_prime, t_exp
    !real(dl) :: Al_mass(l_max_nu_int-1) !CK changed
    !real opacity_nu(EV%nq(1)+1)
    ...
    !FYCR
    next_switch = min(EV%TightSwitchoffTime_nu,tau_switch_ktau, &
         tau_switch_nu_massless,EV%TightSwitchoffTime, tau_switch_nu_massive, &

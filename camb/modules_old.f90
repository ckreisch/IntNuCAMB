    ...
    !FYCR: Maximum l that we are providing the collision term coefficients for
    integer, parameter :: l_max_nu_int = 10
    !FYCR: The number of mass bins that we are providing the collision term coefficients for
    !integer, parameter :: n_mass_bins = 101 !CK
    !FYCR: Number of collision term files provided (for error checking)
    !integer, parameter :: n_files_Al = 3 !CK
    !FYCR: Root of the filename for the files containing collision term coefficients
    character(LEN=20), parameter :: fileroot_coll_coef = 'Coll_rowq_coll_nqmax' !CK
    !FYCR: Name of the file containing the massless Al
    character(LEN=20), parameter :: filename_coll_massless = 'Massless_alpha_l.dat'
    !FYCR: Name of the file containing the neutrino mass vector
    !character(LEN=15), parameter :: filename_mass_vector = 'Mass_vector.dat' !CK
    ...
    real(dl)  :: G_nu_eff, T_nu_0
   
    !FYCR: The coefficient for the collision term of massless neutrinos
    real(dl) :: Al_massless(l_max_nu_int)
    ...
    logical WantReion
    integer nu_i,actual_massless,re_int
    real(dl) nu_massless_degeneracy, neff_i
    external GetOmegak
    real(dl), save :: last_tau0
    !real(dl) :: q1(101,4) !!!!!!MAKE SURE THERE IS A CP VARIABLE FOR THIS
    !real q2(101,4)
    !real q3(101,4)
    ...
    if (CP%Num_nu_massive > 0) then
        ...
        if (CP%share_delta_neff) then
            ...
            !FYCR: Neutrino temperature and number density, massive case
            CP%T_nu_0 = (neff_i)**(1.0d0/4.0d0)*(4.0d0/11.0d0)**(1.d0/3.0d0)*CP%tcmb
            N_nu_now = (actual_massless+CP%Num_Nu_massive)*2.0d0*(3.0d0/11.0d0)* &
                 (1.2020569031595942854_dl/const_pi**2)*CP%T_nu_0**(3.0d0)*(2*const_pi*k_B/(h_P*c))**3
       ...
       !FYCR: Neutrino Temperature and density, massless case
       actual_massless = int(CP%Num_Nu_massless + 1e-6_dl)
       CP%T_nu_0 = (CP%Num_Nu_massless/actual_massless)**(1.0d0/4.0d0)*(4.0d0/11.0d0)**(1.d0/3.0d0)*CP%tcmb
       N_nu_now = (actual_massless)*2.0d0*(3.0d0/11.0d0)* &
            (1.2020569031595942854_dl/const_pi**2)*CP%T_nu_0**(3.0d0)*(2*const_pi*k_B/(h_P*c))**3
    end if
    ...
    if (present(error)) then
        ...
        !write(*,'("has_nu_relativistic  = ",EV)') has_nu_relativistic
        !print*, "has_nu_relativistic", has_nu_relativistic
        !FYCR
        write(*,'("G_eff/G_Fermi        = ",E13.6)') CP%G_nu_eff/1.16637d-11
        if (CP%Num_Nu_Massive > 0) then
    ...
    !FYCR: Load the collision term coefficients for the massless interacting neutrino case
    open(unit=12, file = filename_coll_massless)
    
    !FYCR: Read in the massless coefficients
    do re_int=1,l_max_nu_int-1
       read(12,*) CP%Al_massless(re_int)
    enddo
...
500     if (L< lmax_extrap_highl) &
    ...
    integer, parameter :: nqmax0=10 !maximum array size of q momentum samples
    real(dl) :: nu_q(nqmax0), nu_int_kernel(nqmax0)

    integer nqmax !actual number of q modes evolves

    !FYCR: Coefficients for the collision terms
    !real(dl) :: Al_massive(nqmax0,n_mass_bins,l_max_nu_int-1) !CK
    !CK: Coefficients for collision terms, assuming massive neutrinos decouple while relativistic.
    real(dl) :: Al_massive(nqmax0,l_max_nu_int-1) 
    !FYCR: The values of the a*m_nu/T_nu_0 that we are providing the collision term coefficients for
    !real(dl) :: nu_mass_vec(n_mass_bins) !CK

    public const,Nu_Init,Nu_background, Nu_rho, Nu_drho,  nqmax0, nqmax, &
    nu_int_kernel, nu_q, Al_massive !, nu_mass_vec, Al_mass_interp !CK
    contains
    ...
    if (nqmax > nqmax0) call MpiStop('Nu_Init: nqmax > nqmax0')
    if (nqmax > 5) call MpiStop('Nu_Init error: Number of momentum bins exceeds those in collision term files.') !CK
    if (nqmax > 99) call MpiStop('Nu_Init error: qmax cannot be larger than 99 &
         for the current implementation of massive interacting neutrinos')
    ...
    !CK: Read in massive collision term coefficients for a given point sampling
    if (nqmax==3) then
        filename = fileroot_coll_coef // '3.dat'
	open(unit = fileio_unit+ 1, file = filename)
	do re_int = 1,nqmax
          !FYCR: Read in the massive collision term coeffcients
          read(fileio_unit + 1,*) Al_massive(re_int,1:l_max_nu_int-1)
       	end do
       	close(fileio_unit + 1)
    else if (nqmax==4) then
	filename = fileroot_coll_coef // '4.dat'
	open(unit = fileio_unit+ 2, file = filename)
	do re_int = 1,nqmax
          !FYCR: Read in the massive collision term coeffcients
          read(fileio_unit + 2,*) Al_massive(re_int,1:l_max_nu_int-1)
       	end do
       	close(fileio_unit + 2)     
    else if (nqmax==5) then
	filename = fileroot_coll_coef // '5.dat'
	open(unit = fileio_unit+ 3, file = filename)
	do re_int = 1,nqmax
          !FYCR: Read in the massive collision term coeffcients
          read(fileio_unit + 3,*) Al_massive(re_int,1:l_max_nu_int-1)
       	end do
       	close(fileio_unit + 3)     
    else
	call MpiStop('Nu_Init error: No collision term file for specified nqmax.')
    end if

    !FYCR: Loop to read in the massive collision term coefficients
    !do i=1,nqmax
       !FYCR: Build the filename from the given file root and the index number of the q mode
       !write(file_index,'(I2.2)') i
       !filename = fileroot_coll_coef // file_index // '.dat'
       !FYCR: Load the files containing the collision term coefficients
       !open(unit = fileio_unit+ i, file = filename) 
       !do re_int = 1,n_mass_bins
          !FYCR: Read in the massive collision term coeffcients
          !read(fileio_unit + i,*) Al_massive(i,re_int,1:l_max_nu_int-1)
       !end do
       !FYCR: Close the files
       !close(fileio_unit + i)
    !end do
    
    !FYCR: Load the vector containing the corresponding values of a*m_nu/T_nu_0
    !open(unit=fileio_unit + nqmax +1, file= filename_mass_vector)

    !FYCR: Read in the value of the neutrino mass vector
    !do re_int=1,n_mass_bins
       !read(fileio_unit + nqmax +1,*) nu_mass_vec(re_int)
    !end do
    
    !Close file
    !close(unit=fileio_unit + nqmax +1)
    ...
    !CK: Routine that takes a value of a*m_nu/T_nu_0 and the index
    ! of the momentum bin, and return the value of the collision term coefficient
    !subroutine Al_mass_interp(nqi,actual_mass,Al_mass)
    !  integer nqi,l_mutl,mass_int,row0,row1,p_iter,l_iter
    !  real(dl) mass_sum
    !  real(dl), intent(in) :: actual_mass
     ! real(dl), intent(out) :: Al_mass(l_max_nu_int-1)
     ! real m0,m1
      
      !First, initialize quantities used below
     ! mass_int=0
     ! mass_sum=0._dl

      !First quickly handle very small values of a*m_nu/T_nu_0
     ! if (actual_mass < 1.0d-4) then
     !    Al_mass(1:l_max_nu_int-1) = Al_massive(nqi,1,1:l_max_nu_int-1)
     !    return   
     ! else !now consider the actual massive case
         ! Find which indices in the mass vector we will use for interpolation
      !   do while (mass_sum < actual_mass)
      !      mass_int=mass_int+1
            !We can't exceed the length of the mass vector
      !      if (mass_int > n_mass_bins) then
      !         mass_int = n_mass_bins
      !         exit
      !      end if
      !      mass_sum = nu_mass_vec(mass_int)
      !   enddo
      !end if

      ! Handle the large values of a*m_nu/T_nu_0
      !if (mass_int == n_mass_bins) then
      !   Al_mass(1:l_max_nu_int-1) = Al_massive(nqi,n_mass_bins,1:l_max_nu_int-1)
      !   return   
      !end if

      !We now know that the actual_mass is between nu_mass_vec(mass_int-1) and nu_mass_vec(mass_int) and, hence, m0 and m1
      !if (mass_sum==actual_mass) then
      !   m0=actual_mass
      !   m1=1._dl
      !   row0=mass_int !CK changed mass_int+1 to mass_int
      !   row1=row0+1
      !else
      !   row0=mass_int-1 !CK changed mass_int to mass_int-1
      !   row1=mass_int !CK changed mass_int+1 to mass_int
      !   m0=nu_mass_vec(mass_int-1) !CK changed from mass_sum-0.2
      !   m1=mass_sum
      !endif
      !print *, actual_mass
            
      !linear interpolation for massive collision term 
      !do l_iter=1,l_max_nu_int-1 !CK made more general and revised
      !   Al_mass(l_iter)=real(Al_massive(nqi,row0,l_iter)+(Al_massive(nqi,row1,l_iter)-Al_massive(nqi,row0,l_iter))*(actual_mass-m0)/(m1-m0))
      !enddo
            
    !end subroutine Al_mass_interp
    ...
    !FYCR
    integer,parameter :: nthermo_nu=20000
    ...
    !real(dl) tau_nu_dec,delta_tau_nu_dec  
  ...
  contains
    ...
    !FYCR
    subroutine thermo_nu(tau,opacity_nu, dopacity_nu)
    ...
    !FYCR
    subroutine inithermo_nu(taumin,taumax)  
        use constants
        use precision
        use ModelParams
        ...
        real(dl) actual_massless
        integer  Nu_degeneracy_factor
        external dtauda
        
        !Pre-compute the opacity
        Maxtau_nu=taumax
        tight_tau_nu = 0
        ...
        !  This gives wrong temperature before mediator annihilation, but
        !  the error is harmless as long as it happens before BBN
        !  Determine degeneracy factor for neutrinos
        actual_massless = int(CP%Num_Nu_massless + 1e-6_dl)
        Nu_degeneracy_factor = actual_massless + CP%Num_Nu_massive

        ! Compute the opacity
        dotmu_nu(1) = Nu_degeneracy_factor*Mpc*(CP%G_nu_eff*(1.0d-6*h_P*c/(2*const_pi*eV))**2)**2* &
             (2*const_pi*k_B*CP%T_nu_0/(h_P*c))**5/(a02*a02)
      
        !open(unit=25,file='opacity_neutrinos_1e-3.dat',status='replace')
        do i=2,nthermo_nu
           ...
           !Integrate Friedmann equation using inverse trapezoidal rule.
            ...
            ! Compute the opacity
            dotmu_nu(i) = Nu_degeneracy_factor*Mpc*(CP%G_nu_eff*(1.0d-6*h_P*c/(2*const_pi*eV))**2)**2* &
                 (2*const_pi*k_B*CP%T_nu_0/(h_P*c))**5/(a2*a2)
           

            !Tight coupling switch time when k/opacity is smaller than 1/(tau*opacity)
            if (tight_tau_nu==0 .and. 1/(tau*dotmu_nu(i)) > 0.005) then !0.005
                tight_tau_nu = tau
                print*,'tight_tau_nu/Mpc = ', tight_tau_nu
             end if
                     
           ! if (tight_tau_nu /= 0) print*, tight_tau_nu
!            write(unit=25,fmt=56) 1.d0/a-1.d0,a, dotmu_nu(i),1.0d0/(tau*dotmu_nu(i)),adot/a
!56          format(E14.8,1X,E14.8,1X,E14.8,1X,E14.8,1X,E14.8)
    ...
    !FYCR
    call inithermo_nu(taumin,taumax)
    ...
    !$OMP PARALLEL DO DEFAULT(SHARED),SCHEDULE(STATIC)
    do j2=1,TimeSteps%npoints
        call DoThermoSpline_nu(j2,TimeSteps%points(j2))
    end do
    !$OMP END PARALLEL DO
    ...
    !FYCR
    if(allocated(opac_nu)) then
       deallocate(dopac_nu,opac_nu)
    end if
    allocate(dopac_nu(nstep),opac_nu(nstep))
    ...
    !FYCR
    !cccccccccccccc
    subroutine DoThermoSpline_nu(j2,tau)
    integer j2,i
    real(dl) d,tau
    ...
    d=d-i
    if (i < nthermo_nu) then
        opac_nu(j2)=dotmu_nu(i)+d*(ddotmu_nu(i)+d*(3._dl*(dotmu_nu(i+1)-dotmu_nu(i)) &
        -2._dl*ddotmu_nu(i)-ddotmu_nu(i+1)+d*(ddotmu_nu(i)+ddotmu_nu(i+1) &
        +2._dl*(dotmu_nu(i)-dotmu_nu(i+1)))))
        dopac_nu(j2)=(ddotmu_nu(i)+d*(dddotmu_nu(i)+d*(3._dl*(ddotmu_nu(i+1)  &
        -ddotmu_nu(i))-2._dl*dddotmu_nu(i)-dddotmu_nu(i+1)+d*(dddotmu_nu(i) &
        +dddotmu_nu(i+1)+2._dl*(ddotmu_nu(i)-ddotmu_nu(i+1))))))/(tau &
        *dlntau_nu)
    else
        opac_nu(j2)=dotmu_nu(nthermo_nu)
        dopac_nu(j2)=ddotmu_nu(nthermo_nu)
    end if
    end subroutine DoThermoSpline_nu

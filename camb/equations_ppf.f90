    ...
    ! Apr 2014: FYCR: Updated for interacting massless neutrinos
    ! Apr 2016: CK: Updated for interacting massive neutrinos
...
1   if (khi-klo.gt.1) then
    ...
    !Vector mode shear
    integer, parameter :: max_l_evolve = 1024 !256 !CK update !Maximum l we are ever likely to propagate
	...
	real(dl) has_nu_relativistic_pir_pert	!CK update	
        real(dl) pir_massive(max_nu,nqmax0) !CK update
 	...
 	!FYCR		
        logical TightCoupling_nu		
        real(dl) TightSwitchoffTime_nu
    ...
    integer ind, nu_i, ind3,inq3,iq_iter,mass_int,l_iter !CK update
    !FYCR
    real(dl) cs2, opacity, dopacity, aq,v, opacity_nu !, actual_mass
    ...
    real(dl) noSwitch, smallTime, tau_p, tau_prime, t_exp !CK update
    ...
    if (DoLateRadTruncation) then
        if (.not. EV%no_nu_multpoles) & !!.and. .not. EV%has_nu_relativistic .and. tau_switch_nu_massless ==noSwitch)  &
            tau_switch_no_nu_multpoles= max(15/EV%k_buf*AccuracyBoost,min(taurend,matter_verydom_tau),2*EV%TightSwitchoffTime_nu) !FYCR
    ...
    !FYCR
    next_switch = min(EV%TightSwitchoffTime_nu,tau_switch_ktau, tau_switch_nu_massless,EV%TightSwitchoffTime, tau_switch_nu_massive, &
	...
	!FYCR
	   ...
	   !Neutrino tight coupling		
           ...
           y(EV%r_ix+3) = 3./7*y(EV%r_ix+2)*EV%k_buf/(opacity_nu*CP%Al_massless(2))		
           if(EV%has_nu_relativistic) then		
              y(EV%nu_pert_ix+2) = EV%has_nu_relativistic_pir_pert		
              y(EV%nu_pert_ix+3) = (3./7)*(EV%k_buf/(CP%Al_massless(2)*opacity_nu))*& 		
                   (EV%has_nu_relativistic_pir_pert+ y(1)**2*EV%pir)		
           end if		
           do nu_i = 1, CP%Nu_mass_eigenstates		
              ind3 = EV%nu_ix(nu_i)		
              do inq3=1,EV%nq(nu_i)		
		 if (inq3>nqmax) then		
	      	 	call MpiStop('Perturbed hierarchy error: q mode larger than nqmax, so no collision term available.')		
	         end if		
                 aq=y(1)*nu_masses(nu_i)/nu_q(inq3)		
                 v=1._dl/sqrt(1._dl+aq*aq)		
                 y(ind3+2) = EV%pir_massive(nu_i,inq3) 		
		 y(ind3+3) = (3./7)*(EV%k_buf/(Al_massive(inq3,2)*opacity_nu))*EV%pir_massive(nu_i,inq3)*v !CK		
                 ind3 = ind3 + EV%lmaxnu_tau(nu_i) + 1 		
              end do		
           end do		
    ...
    do nu_i=1, CP%Nu_Mass_eigenstates
        ...
        a_mass =  1.e-3_dl/nu_mass/lAccuracyBoost  !FYCR changed 0.1 for 0.001
        ...
        do j=2,nqmax
            ...
            time= time !+ DeltaTimeMaxed(nu_q(j-1)*a_mass,nu_q(j)*a_mass, 0.01_dl) !CK update
	...
	!FYCR
        if (EV%TightCoupling_nu) then		
            neq=neq + 2		
        else if (EV%high_ktau_neutrino_approx) then
            neq=neq + 3
        else
            neq=neq + (EV%lmaxnr+1)
        end if
    ...
    if (CP%Num_Nu_massive /= 0) then
        ...
        do nu_i=1, CP%Nu_Mass_eigenstates
            if (EV%high_ktau_neutrino_approx) then !CK update
            ...
            !if (nu_masses(nu_i) > 5000 .and. CP%Transfer%high_precision) EV%lmaxnu_tau(nu_i) = EV%lmaxnu_tau(nu_i)*2 !megadamping !CK update
            ...
            if (EV%MassiveNuApprox(nu_i)) then
                neq = neq+4
            !FYCR		
          	elseif(EV%TightCoupling_nu) then		
             	neq = neq + EV%nq(nu_i)*2		
          	else		
    ...
    if (.not. EV%no_nu_multpoles .and. .not. EVout%no_nu_multpoles) then
		...
		if(EV%TightCoupling_nu .or. EVOut%TightCoupling_nu) then
			lmax=1
        else if (EV%high_ktau_neutrino_approx .or. EVout%high_ktau_neutrino_approx) then
            lmax=2
        else
    ...
    if (CP%Num_Nu_massive /= 0) then
        do nu_i=1,CP%Nu_mass_eigenstates
            ...
            if (EV%MassiveNuApprox(nu_i) .and. EVout%MassiveNuApprox(nu_i)) then
               ...
               !FYCR
               ! Copy the already present momentum modes 
               do i=1,nq
                  if(EVOut%TightCoupling_nu .and. EV%TightCoupling_nu) then
                     ind= ix_off + (i-1)*(1+1)
                     ind2=ix_off2+ (i-1)*(1+1)
                  else if(.not. EVOut%TightCoupling_nu .and. EV%TightCoupling_nu) then
                     ind= ix_off + (i-1)*(1+1)
                  ...
                  !print*, 'massive #',i, ind, ind2,EV%lmaxnu_tau(nu_i),EV%TightCoupling_nu, EVOut%TightCoupling_nu
                  yout(ind2:ind2+lmax) = y(ind:ind+lmax)
               end do
               !FYCR
               ! Add the new momentum modes (from the massless hierarchy + first-order expansion in m^2/q^2)
               do i=nq+1, EVOut%nq(nu_i)
                  ...
                  !print*, 'massive first #',i,EV%r_ix ,ind2,EV%lmaxnu_tau(nu_i),EV%TightCoupling_nu, EVOut%TightCoupling_nu
                  !Notice how the massless hierarchy enters the massive one here
	...
	!FYCR		
    real(dl) pir, pir_pert,pir_massive
    integer iq, ind
    ...
    do iq=1,EV%nq(nu_i)
		!FYCR
		if (EV%tightcoupling_nu) then
			pir_massive = EV%pir_massive(nu_i,iq)
		else
			pir_massive= y(ind+2)
		end if
        aq=am/nu_q(iq)
        ...
        if (present(dpnu)) then
            ...
            pinu=pinu+ nu_int_kernel(iq)*pir_massive*v !CK update
        end if
		!FYCR
		if (EV%tightcoupling_nu) then		
        	ind=ind+2		
    ...
    do iq=EV%nq(nu_i)+1,nqmax
		...
		if (EV%tightcoupling_nu) then
			pir = EV%pir
			pir_pert = EV%has_nu_relativistic_pir_pert
		else
			pir = y(EV%r_ix+2)
			pir_pert = y(ind+2)
		end if
        ...
        if (present(dpnu)) then
            dpnu=dpnu+ tmp*v
			!FYCR
            pinu = pinu+ nu_int_kernel(iq)*(pir+ pert_scale*pir_pert)*v
        end if
    end do
	...
	!FYCR		
    real(dl) pir_massive,pir_massive_dot,pir,pir_pert,pir_dot,pir_pert_dot
    integer iq,ind
	...
	if (EV%tightcoupling_nu) then
		pir_massive = EV%pir_massive(nu_i,iq)
		pir_massive_dot = 0.0 !!FYCR: ADD THIS TERM?
	else
		pir_massive= y(ind)
		pir_massive_dot = ydot(ind)
    end if		
       	q=nu_q(iq)		
       	aq=am/q		
       	aqdot=aq*adotoa		
       	...
       	!FYCR		
       	pinudot=pinudot+nu_int_kernel(iq)*(pir_massive_dot*v+pir_massive*vdot)		
       	!FYCR		
       	if (EV%tightcoupling_nu) then		
          	ind=ind+2		
       	else		
    ...
    do iq=EV%nq(nu_i)+1,nqmax
		...
		if (EV%tightcoupling_nu) then
			...
			pir_dot = 0.0
			pir_pert = EV%has_nu_relativistic_pir_pert
			pir_pert_dot = 0.0  !!FYCR: ADD THIS TERM?
		else
			pir = y(EV%r_ix+2)
			pir_dot = ydot(EV%r_ix+2)
			pir_pert = y(ind)
			pir_pert_dot = ydot(ind)		
		end if		
		q=nu_q(iq)		
		aq=am/q		
		aqdot=aq*adotoa		
		...
		v=1._dl/sqrt(1._dl+aq*aq)		
		vdot=-aq*aqdot/(1._dl+aq*aq)**1.5d0		
		psi2dot= pir_dot + pert_scale*pir_pert_dot		
		psi2=pir + pert_scale*pir_pert		
    ...
    real(dl) :: am
    real(dl) pinu,q,aq,v,pir_massive
    integer iq, ind
    ...
    do iq=1, EV%nq(nu_i)
		...
		q=nu_q(iq)		
       	aq=am/q		
       	v=1._dl/sqrt(1._dl+aq*aq)		
       	pinu=pinu+nu_int_kernel(iq)*pir_massive*v		
	...
	!CK
	!real(dl) Psi_field, Phi_field, function_mu, function_gamma, xi, sumpotdot
    ...
    !FYCR		
    if (EV%TightCoupling_nu) then		
       pirdot = -dopac_nu(j)/opac_nu(j)*pir + (8._dl/15._dl)*k/(CP%Al_massless(1)*opac_nu(j))*(-2.0_dl*adotoa*sigma  &		
            +etak/EV%Kf(1)-  dgpi/k + 3.0_dl*qrdot/4.0_dl)
    ...
    if (CTransScal%NumSources > 2) then
        ...
        if (tau > taurend .and. CP%tau0-tau > 0.1_dl) then !CK update tau_maxvis
    ...
    !CK to compute potentials
	!Psi_field = (-2*adotoa*sigma-(grhog_t*pig+grhor_t*pir)/k+etak)/k + adotoa*sigma/k
	!Phi_field = etak/k - adotoa*sigma/k
	!function_mu = -2._dl*k*((-2*adotoa*sigma-(grhog_t*pig+grhor_t*pir)/k+etak)+adotoa*sigma)/(dgrho)
	!function_gamma = (etak - adotoa*sigma)/((-2*adotoa*sigma-(grhog_t*pig+grhor_t*pir)/k+etak) + adotoa*sigma)
	!xi = vq/(adotoa*clxq)
	!!sumpotdot = phidot + psidot
	!sumpotdot = (4.D0/3.D0*k*EV%Kf(1)*sigma+(-2.D0/3.D0*sigma-2.D0/3.D0*etak/adotoa)*k &
	!	-diff_rhopi/k**2-1.D0/adotoa*dgrho/3.D0+(3.D0*gpres+5.D0*grho)*sigma/k/3.D0 &
	!	-2.D0/k*adotoa/EV%Kf(1)*etak)
	!write (10,'(10e15.5)') a, tau, GR_Psi, GR_Phi, GR_function_mu, GR_function_gamma, GR_xi
	!write (76,'(10e15.5)') a, tau, sigma, Psi_field, Phi_field, function_mu, function_gamma, xi, sumpotdot
    ...
    !initv(1,i_qr)=-chi*EV%Kf(1)*(4*Rv+23)/Rp15*x3/27 !CK update
	...
	!FYCR no free-streaming initial conditions
	initv(1,i_qr)=initv(1,i_qg)
	initv(1,i_pir)=0.0_dl		
    initv(1,i_aj3r)=0.0_dl		
    ...
    do nu_i = 1, CP%Nu_mass_eigenstates
        ...
        do  i=1,EV%nq(nu_i)
			...
			if (EV%tightcoupling_nu) then !CK update
   				ind = ind + 2		
            else		
               	ind = ind + EV%lmaxnu_tau(nu_i)+1		
            end if
	...
	!FYCR
	real(dl) opacity_nu,dopacity_nu, pir_pert,pir_massive
    real(dl) clxq, vq,  E2, dopacity
    integer l,i,ind, ind2, off_ix, ix,l_int,lm,less_int
   ...
   !FYCR: Get neutrino opacity		
    ...
    if (EV%no_nu_multpoles) then
        ...
        qr  =ay(EV%r_ix+1)
       	!FYCR
        if (.not. EV%TightCoupling_nu) pir =ay(EV%r_ix+2)
    endif
    ...
    if (EV%TightCoupling) then
        ...
        !FYCR		
        if (EV%tightcoupling_nu) pir = (8.0/15.0)/(CP%Al_massless(1)*opacity_nu)*k*(sigma+3._dl*qr/4._dl) 
    ...
    !FYCR		
    if (EV%tightcoupling_nu) EV%pir = (8.0/15.0)/(CP%Al_massless(1)*opacity_nu)*k*(sigma+3._dl*qr/4._dl)
		    ...
		    if (EV%high_ktau_neutrino_approx) then
		        ...
		        if (EV%lmaxnr>2) then
                	...
                	pirdot=EV%denlk(2)*qr- EV%denlk2(2)*ay(ix+1)+8._dl/15._dl*k*sigma &		
                     -opacity_nu*pir*CP%Al_massless(1)
		            ayprime(ix)=pirdot
					less_int = 1
		            do l=3,EV%lmaxnr-1
		                ix=ix+1
						if (l > l_max_nu_int) then
							less_int=l_max_nu_int-1
						else
							less_int=less_int+1
						end if
		                !FYCR		
                   		ayprime(ix)=(EV%denlk(l)*ay(ix-1) - EV%denlk2(l)*ay(ix+1))-ay(ix)*opacity_nu*CP%Al_massless(less_int)
		            end do
		            ...
		            ayprime(ix)=k*ay(ix-1)- (EV%lmaxnr+1)*cothxor*ay(ix)-ay(ix)*opacity_nu*CP%Al_massless(less_int)		
             	else		
                	!FYCR		
                	pirdot=EV%denlk(2)*qr +8._dl/15._dl*k*sigma-opacity_nu*pir*CP%Al_massless(1)
		            ayprime(ix)=pirdot
		        end if !EV%lmaxnr>2
    ...
    do nu_i = 1, CP%Nu_mass_eigenstates
        if (EV%MassiveNuApprox(nu_i)) then
            ...
            ind=EV%nu_ix(nu_i)
            do i=1,EV%nq(nu_i) !Loop over momentum states
                q=nu_q(i)
				...
				!FYCR
				if (EV%tightcoupling_nu) then
					EV%pir_massive(nu_i,i) = (8.0/15.0)/(opacity_nu*Al_massive(i,1))*k*(sigma+3._dl*ay(ind)*v/4._dl)
					pir_massive = EV%pir_massive(nu_i,i)
				else
					pir_massive = ay(ind+1)
				end if
				ayprime(ind)=v*(EV%denlk(1)*ay(ind-1)-EV%denlk2(1)*pir_massive)
				ind=ind+1
				!FYCR		
              	if (.not. EV%tightcoupling_nu) then
		            if (EV%lmaxnu_tau(nu_i)==2) then
	                    ayprime(ind)=-ayprime(ind-2) -3*cothxor*ay(ind) &		
                         -ay(ind)*opacity_nu*Al_massive(i,1) 		
                 	else		
                    	ayprime(ind)=v*(EV%denlk(2)*ay(ind-1)-EV%denlk2(2)*ay(ind+1)) + k*8._dl/15._dl*sigma &		
                         -ay(ind)*opacity_nu*Al_massive(i,1) 	
		                do l=3,EV%lmaxnu_tau(nu_i)-1
		                    ind=ind+1
							if (l>l_max_nu_int) then
		                    	l_int=l_max_nu_int-1 		
                       		else		
                          		l_int=l-1		
                       		endif		
                       		ayprime(ind)=v*(EV%denlk(l)*ay(ind-1)-EV%denlk2(l)*ay(ind+1)) &		
                            -ay(ind)*opacity_nu*Al_massive(i,l_int)
		                end do
		                !  Truncate moment expansion.
		                ind = ind+1
		                ayprime(ind)=k*v*ay(ind-1)-(EV%lmaxnu_tau(nu_i)+1)*cothxor*ay(ind)&
							-ay(ind)*opacity_nu*Al_massive(i,l_max_nu_int-1) 
		            end if
                	ind = ind+1
				end if
            end do
	...
	!FYCR
    if (EV%has_nu_relativistic) then
		! Define index for the perturbed hierarchy
        ind=EV%nu_pert_ix
		! Define index for the normal massless neutrino hierarchy
        ind2= EV%r_ix
		! This is the l=0 perturbed hierarchy equation 
		ayprime(ind)=+k*a2*qr -k*ay(ind+1)
		!Increase the index
		ind = ind+1
		ind2 = ind2+1
        ! This is the l=1 perturbed hierarchy equation
		l=1
       	if (EV%tightcoupling_nu) then		
          	pir_pert = (2.0/5)/(CP%Al_massless(1)*opacity_nu)*k*(ay(ind)+a2*qr)		
          	EV%has_nu_relativistic_pir_pert = pir_pert		
       	else		
          	pir_pert = ay(EV%nu_pert_ix+2)		
       	end if		
       		
       	ayprime(ind)= -a2*(EV%denlk(l)*ay(ind2-1)-EV%denlk2(l)*pir) &		
            +   (EV%denlk(l)*ay(ind-1)-EV%denlk2(l)*pir_pert)		
       	! Do the loop over the perturbed hierarchy l>=2 moments		
       	if (.not. EV%tightcoupling_nu) then		
          	do l=2,EV%lmaxnu_pert-1		
             	ind=ind+1		
             	ind2=ind2+1		
             	if (l > l_max_nu_int) then		
                	ml_int= l_max_nu_int - 1		
             	else		
                	ml_int=l-1		
             	endif		
             	ayprime(ind)= -a2*(EV%denlk(l)*ay(ind2-1)-EV%denlk2(l)*ay(ind2+1)) &		
                  +   (EV%denlk(l)*ay(ind-1)-EV%denlk2(l)*ay(ind+1)) - ay(ind)*opacity_nu*CP%Al_massless(ml_int)		
          	end do		
         	ind=ind+1		
          	ind2=ind2+1		
          	! Set the index for the truncation		
          	if ( ml_int < l_max_nu_int-1) then		
             	ml_int= ml_int + 1		
          	else		
             	ml_int= l_max_nu_int-1		
          	end if		
          	! Truncate the hierarchy		
          	ayprime(ind)= k*(ay(ind-1) -a2*ay(ind2-1)) -(EV%lmaxnu_pert+1)*cothxor*ay(ind)- ay(ind)*opacity_nu*CP%Al_massless(ml_int)		
       	end if

    ...
    !Check if we are starting early enough for the neutrinos
    T_nu_dec_approx = 100d0*(1.0d6*eV/k_B)*(89.948433298516998384_dl/(1.221d22*CP%G_nu_eff**2* &
         (CP%Num_Nu_Massless+CP%Num_Nu_Massive)))**(1.0d0/3.0d0)
    ...
    !print*, tau_nu_dec
    if (tau_nu_dec < taumin) taumin = 0.01d0*tau_nu_dec

    !Store value for checking when we solve for each k mode
    taumin_check = taumin

    !taumin=GetTauStart(maxq) !CK update
    ...
    integer j,ind,itf !,hold
    real(dl) c(24),w(EV%nvar,9), y(EV%nvar), sources(SourceNum)

    real(dl) yprime(EV%nvar), ddelta, delta, adotoa,dtauda, growth
    real(dl) a,dgrho,zevol,dgq,dgs,sigma,grho_nu,gpres_nu,dgpi_nu,gdpi_diff_nu,pidot_sum_nu
    real(dl) de_nu,theta_nu,pi_nu,dgq_nu,dgrho_nu,pi_gam,phi,psi
    real(dl) convert_delta, convert_theta,a2,convert_delta_numass
    real(dl) de_nu_nomass,theta_nu_nomass,pi_nu_nomass
    external dtauda
    ...
    !write(*,*) 'taustart: ', taustart
    !write(*,*) 'tol: ', tol

    !!Example code for plotting out variable evolution
    if (fixq/=0._dl) then
       tol1=tol/exp(AccuracyBoost-1)
       !open(unit=35,file='Int_mass_nu_evol_var_k_40.txt',status='replace')
       open(unit=35,file='/Users/ckreisch/Documents/research/neutrinos/InteractingNeutrinos/sterile_mixing/updatetoNov15/evolution/final/Gneg2_mpt23_kpt1.dat',status='replace')
       do j=1,10000
          tauend = taustart * exp(j/10000._dl*log(CP%tau0/taustart)) !CK changed 10000 t0 15138
          ...
          pidot_sum_nu = 0.0
          if (CP%Num_Nu_Massive /= 0) then
             call MassiveNuVarsOut(EV,y,yprime,a,grho_nu,gpres_nu,dgrho_nu,dgq_nu,dgpi_nu, gdpi_diff_nu,pidot_sum_nu)
             de_nu = dgrho_nu/grho_nu
             theta_nu = dgq_nu/grho_nu
             pi_nu = dgpi_nu/grho_nu
          end if

          de_nu_nomass = y(EV%r_ix)
          theta_nu_nomass = y(EV%r_ix+1)
          if (EV%TightCoupling_nu) then
             pi_nu_nomass = EV%pir
          else
             pi_nu_nomass = y(EV%r_ix+2)
          end if
          ...
          adotoa=sqrt((grho_nu+grhob/a+grhoc/a + grhog/a**2+grhornomass/a**2+grhov*a**2)/3)
          dgrho=dgrho_nu + grhob/a*y(4)+grhoc/a*y(3) + grhog/a**2*y(EV%g_ix)+grhornomass/a**2*y(EV%r_ix)
          dgq= grhob/a*y(5) + grhog/a**2*y(EV%g_ix+1)+grhornomass/a**2*y(EV%r_ix+1)+dgq_nu
          zevol=(0.5_dl*dgrho/EV%q + y(2))/adotoa
          sigma=(zevol+1.5_dl*dgq/EV%q2)
          convert_delta = sigma*adotoa/(EV%q)
          convert_delta_numass = sigma*adotoa*(grho_nu+gpres_nu)/(EV%q*grho_nu)
          convert_theta = EV%q*sigma
          phi = (y(2)-adotoa*sigma)/EV%q
          dgs = dgpi_nu + grhog/a**2*pi_gam+grhornomass/a**2*pi_nu_nomass
          psi = phi - dgs/EV%q2

	  !j=j+1	!CK
	  !write(*,*) 'j: ', j

          ! Output variables in Newtonian Gauge, including the potentials Phi and Psi
          !write (35,'(15E18.9)') tau, y(1), y(3)-3.0_dl*convert_delta, y(4)-3.0_dl*convert_delta, &
          !     0.75_dl*(y(EV%g_ix)-4.0_dl*convert_delta), &
          !     0.75_dl*EV%q*y(EV%g_ix+1)+convert_theta,pi_gam,de_nu-3.0_dl*convert_delta_numass, &
          !     0.75_dl*EV%q*theta_nu+convert_theta, &
          !     pi_nu, 0.75_dl*(de_nu_nomass-4.0_dl*convert_delta), &
          !     0.75_dl*EV%q*theta_nu_nomass+convert_theta,pi_nu_nomass,phi,psi
	  !CK to force GR timesteps to be the same
          !if (tau>=0.1 .and. hold==0) then
		!write(*,*) 'if statement'
		!taustart = tau
		!j = 2
		!hold=1
		!write (35,'(15E18.9)') a, tau, sigma, phi, psi
	  !else if (tau>0.1) then
		!write(*,*) 'write'
	  	write (35,'(15E18.9)') a, tau, sigma, phi, psi
	  !end if
